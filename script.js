function onOpen(e) {
 ...
 var body = DocumentApp.getActiveDocument().getBody();
  ...
  // Clear the text surrounding "Argument Inro:", with or without text.
  //body.replaceText("^.*Start ?Argument.*$", "I would like you to take a moment and consider the following argument:");
  
  // Use editAsText to obtain a single text element containing all the characters in the document.
  var text = body.editAsText();

  var q = 'pigeons';
  var response = UrlFetchApp.fetch('https://api.duckduckgo.com/?q=' + q + '&format=json');
  var result = JSON.parse(response.getContentText());
  Logger.log(result['RelatedTopics'][0]['Text']); 
  
  // Insert text at the beginning of the document.
  //var start = text.insertText(0, 'According to leading sources on ' + q + ', ');
  var start = text.insertText(0, 'According to the Britannica Encyclopedia section on ' + q + ', ');
